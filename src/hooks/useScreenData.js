...
export default function () {
  ...
  const growthLastDay = ref(0) //每日增长率百分比初始值
  const growthLastMonth = ref(5)
  ...
  onMounted(()=>{
    ...
    _riderData.rateData.data1=_riderData.orderData.data1.map(item=>{
      item = item+rondom(100)
      return item
    })
    _riderData.rateData.data1=_riderData.orderData.data1.map(item=>{
    ...
    _riderData.rateData.data2=_riderData.orderData.data2.map(item=>{
      item = item+rondom(100)
      return item
    })
    _riderData.rateData.data2=_riderData.orderData.data2.map(item=>{
