<template>
  <div class="sales-list">
    <base-scroll-list1 :config="config" />
  </div>
</template>

<script>
import { ref, onMounted, watch } from "vue";
import BaseScrollList1 from "./BaseScrollList/BaseScrollList1.vue";
export default {
  name: "SalesList",
  components: { BaseScrollList1 },
  props: {
    data: Array,
  },
  setup(props) {
    ...
    const update = () => {
      ...
      // 数据处理逻辑
      for (let i = 0; i < props.data.length; i++) {//定义序列号样式
        data[i] = [];
        if (i % 2 === 0) {
          headerIndexData[i] = `
